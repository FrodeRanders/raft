...
echo "Starting demo setup with opinionated defaults"
...
if [[ "${RAFT_DEMO_AUTORUN}" == "1" ]]; then
    echo
    echo "Running automated reconfiguration demo"
    "${ROOT_DIR}/scripts/demo_reconfigure.sh"
    ...
    echo "  ${ROOT_DIR}/scripts/demo_timeline.sh ${RAFT_LOG_DIR}"
fi
