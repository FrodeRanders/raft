...
- [x] Add mixed Java/C++ static-cluster scenarios.
- [x] Add C++ client workload coverage.
- [x] Add C++ joining-node membership coverage.
- [x] Add C++ leader membership-admin coverage.
...
- `Harness documentation`: the Clojure harness now has code comments around the main Jepsen extension points: DB lifecycle, client classification, generators, checkers, nemeses, local remote control, and JSONL observation capture.
...
- `Mixed Java/C++ clusters`: the `mixed` suite validates Java/C++ static clusters, Java CLI and C++ CLI workload clients, target-matched Java/C++ client dispatch, C++ joining learners, and C++ leader membership administration.
- `C++ membership-admin parity`: C++ nodes now serve `reconfiguration-status`, preserve `/learner` roles in reconfigure requests, report stable role transitions, and support join admission plus learner promotion in mixed Jepsen membership runs.
