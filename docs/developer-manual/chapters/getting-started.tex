...
\begin{itemize}
\item \filepath{docker/compose-srv-java.yml} --- 3 Java nodes + CoreDNS
\item \filepath{docker/compose-srv-cpp.yml} --- 3 C++ nodes + CoreDNS
\item \filepath{docker/compose-srv-rust.yml} --- 3 Rust nodes + CoreDNS
\item \filepath{docker/compose-srv-mixed.yml} --- 1 Java + 1 C++ + 1 Rust
\end{itemize}
...
discoverable service endpoint (\texttt{\_raft.\_tcp.raft.\textless
ns\textgreater .svc.cluster.local}). The same mechanism works in Docker
...
\begin{lstlisting}[language=yaml,caption={Kubernetes headless Service}]
apiVersion: v1
kind: Service
metadata:
  name: raft
