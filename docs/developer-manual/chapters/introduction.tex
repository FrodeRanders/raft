...
made safe, how commands and snapshots are encoded, and what the Java and C++
integration surfaces expect.
...
and cross-language parity. The Java and C++ implementations are intended to be
...
\item Sections~\ref{sec:JavaLibrary} and~\ref{sec:CppLibrary} describe the Java and C++ integration surfaces.
...
\item support Java and C++ applications through equivalent protocol behavior.
\end{itemize}

The Java implementation is the broader implementation surface. The C++
implementation is developed as a peer implementation and should be brought up to
the same behavioral maturity where differences affect Raft semantics. Domain
examples may differ temporarily, but the Raft machinery should not.
