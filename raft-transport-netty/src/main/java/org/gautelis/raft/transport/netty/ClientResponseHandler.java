        ...
        switch (type) {
            ...
            default -> {
                if (null != messageHandler) {
                    MessageResponder responder = (replyCorrelationId, replyType, replyPayload) ->
                            ctx.writeAndFlush(ProtoMapper.wrap(replyCorrelationId, replyType, replyPayload));
                    messageHandler.handle(correlationId, type, payload, responder);
                }
            }
        }
    }
