 ...
 */
public class RaftClient implements RaftTransportClient {
    ...
    public CompletableFuture<List<VoteResponse>> requestVoteFromAll(
            Collection<Peer> peers,
            VoteRequest req
    ) {
        CompletableFuture<List<VoteResponse>> promise = new CompletableFuture<>();
    ...
    private void checkDone(
            ...
            CompletableFuture<List<VoteResponse>> promise
    ) {
        int current = count.incrementAndGet();
        if (current == expectedResponses) {
            promise.complete(new ArrayList<>(responses));
        }
    }
