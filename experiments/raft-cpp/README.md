...
The current `install-snapshot` command sends a single-chunk request with:

- `offset=0`
- `done=true`
- optional `snapshot_data`

That is still not a multi-chunk snapshot-transfer implementation, but it is now semantic enough for the bounded prototype to:
