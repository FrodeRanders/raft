...
namespace {
    ...
    std::vector<raftcpp::InMemoryRpcHandler::Endpoint> endpoints_from_peers(
    ...
    [[noreturn]] void run_stateful_server(
        ...
        boost::asio::io_context io_context;
        auto handler = std::make_shared<raftcpp::InMemoryRpcHandler>(peer_id, current_term, last_log_index,
                                                                     last_log_term);
    ...
    [[noreturn]] void run_active_server(
        ...
        std::uint16_t port,
        const std::string &,
        std::shared_ptr<raftcpp::RaftNode> node,
        std::shared_ptr<raftcpp::RpcHandler> handler,
        std::vector<raftcpp::PeerEndpoint> peers,
        ...
        if (auto in_memory = std::dynamic_pointer_cast<raftcpp::InMemoryRpcHandler>(handler)) {
            ...
            in_memory->set_join_tracker(
                [&runtime](const std::string &peer_id, const raftcpp::InMemoryRpcHandler::Endpoint &endpoint) {
            ...
            in_memory->set_membership_updater(
                [&runtime](const std::vector<std::string> &peer_ids,
                           const std::vector<raftcpp::InMemoryRpcHandler::Endpoint> &endpoints) {
                    std::vector<raftcpp::PeerEndpoint> peers;
    ...
    [[noreturn]] void run_active_persistent_server(
        ...
        run_active_server(
            ...
            handler,
            std::move(peers),
            [handler](const raftcpp::RaftNode &current_node) {
    ...
    int run_election_round(
        ...
        const auto became_leader = runtime.run_election_round();
        std::cout
                << "role: " << (runtime.node().role() == raftcpp::RaftNode::Role::leader
                                    ? "leader"
    ...
    int run_replicate_once_persistent(
        ...
        raftcpp::RaftRuntime runtime(
            ...
            node,
            std::move(peers),
            [&store](const raftcpp::RaftNode &current_node) {
