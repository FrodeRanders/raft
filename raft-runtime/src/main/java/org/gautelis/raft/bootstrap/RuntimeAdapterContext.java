...
public record RuntimeAdapterContext(
        ...
        ClientCommandAuthenticator authenticator,
        RaftTransportFactory transportFactory
) {
    public RuntimeAdapterContext {
        if (configuration == null) {
            throw new IllegalArgumentException("configuration must not be null");
        }
        if (authenticator == null) {
            authenticator = ClientCommandAuthenticator.none();
        }
    ...
    public Set<String> commandAllowList() {
        return configuration.commandAllowList();
    }
