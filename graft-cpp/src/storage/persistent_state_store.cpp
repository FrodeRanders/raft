...
namespace graft {
    PersistentStateStore::PersistentStateStore(std::filesystem::path path)
        : path_(std::move(path)) {
    }

    const std::filesystem::path &PersistentStateStore::path() const {
        return path_;
    }

    bool PersistentStateStore::exists() const {
        return std::filesystem::exists(path_);
    }

    void PersistentStateStore::save(const RaftNode::PersistentState &state) const {
        ...
        out << "reconfiguration_started_at_millis=" << state.reconfiguration_started_at_millis << '\n';
    ...
    std::optional<RaftNode::PersistentState> PersistentStateStore::load() const {
        if (!exists()) {
            return std::nullopt;
        }
        ...
        while (std::getline(in, line)) {
            ...
            } else if (key == "reconfiguration_started_at_millis") {
                state.reconfiguration_started_at_millis = std::stoll(value);
    ...
    std::string PersistentStateStore::escape(const std::string &value) {
        std::string out;
    ...
    std::string PersistentStateStore::unescape(const std::string &value) {
        std::string out;
        out.reserve(value.size());
        bool escaped = false;
        for (const char ch: value) {
