...
namespace graft {
    RaftRuntime::RaftRuntime(boost::asio::io_context &io_context, RaftNode::Config config,
                             std::vector<PeerEndpoint> peers)
        : RaftRuntime(io_context, std::make_shared<RaftNode>(std::move(config)), std::move(peers), {}) {
    }

    RaftRuntime::RaftRuntime(boost::asio::io_context &io_context, std::shared_ptr<RaftNode> node,
                             std::vector<PeerEndpoint> peers)
    ...
    RaftNode &RaftRuntime::node() {
        return *node_;
    }

    const RaftNode &RaftRuntime::node() const {
        return *node_;
    }

    std::shared_ptr<RaftNode> RaftRuntime::node_ptr() {
        return node_;
    }

    std::vector<PeerEndpoint> RaftRuntime::peers() const {
        std::scoped_lock lock(peers_mu_);
        return peers_;
    }

    void RaftRuntime::configure_peers(std::vector<PeerEndpoint> peers) {
        std::vector<std::string> peer_ids;
    ...
    void RaftRuntime::track_peer(PeerEndpoint peer) {
        if (peer.peer_id.empty()) {
            throw std::runtime_error("peer id is required");
        }
        std::scoped_lock lock(peers_mu_);
    ...
    std::optional<std::string> RaftRuntime::replicate_entry_once_with_result(const std::string &data) {
        ...
        // Client success is tied to the command being committed and applied, not merely appended locally.
        // The returned result is read from the state machine after replication has advanced the commit index.
        std::size_t successes = 0;
        for (const auto &peer: peers()) {
            successes += replicate_peer_until_caught_up(peer) ? 1 : 0;
        }
    ...
    std::vector<PeerEndpoint> RaftRuntime::voting_peers() const {
    ...
    bool RaftRuntime::sync_peer_once(const PeerEndpoint &peer) {
        ...
        // A follower whose next index has fallen behind the compacted prefix cannot be repaired with
        // AppendEntries. Send InstallSnapshot first, then resume normal log replication from the snapshot edge.
    ...
    bool RaftRuntime::replicate_peer_until_caught_up(const PeerEndpoint &peer) {
        ...
        for (std::size_t attempt = 1; attempt <= kMaxReplicationAttempts; ++attempt) {
            ...
            // Failed AppendEntries responses decrement next_index one step at a time in RaftNode. Retrying here
            // drives that backtracking loop until the follower's log prefix matches the leader's.
            if (node_->snapshot_index() > 0 && next_index <= node_->snapshot_index()) {
    ...
    bool RaftRuntime::send_snapshot_to_peer(const PeerEndpoint &peer, std::size_t attempt) {
        ...
        // Deliberately small chunks keep the smoke tests exercising multi-request snapshot transfer instead of
        // only the trivial single-frame path.
        for (std::size_t offset = 0; offset < total_size; offset += kSnapshotChunkBytes) {
            ...
            try {
                ...
                const auto advanced = chunk_request.done() && node_->handle_install_snapshot_response(
